...
public class AsyncClient extends XlogClient {
  ...
  public boolean doSend(LogData[] logDatas) {
    if (logDataBQ.size() == cacheQueueCapacity) {
      ...
      boolean res = false;
      synchronized (logDataBQ) {
        res = logDataBQ.add(logDatas);
        logDataBQ.notify();
      }
      return res;
  ...
  private class Sender extends Thread {
    public void run() {
      ...
      while (true) {
        try {
          synchronized (logDataBQ) {
            if (logDataBQ.isEmpty()) {
              logDataBQ.wait();
            }
            logDatas = logDataBQ.remove(0);
          }
  ...
  public void initialize(String cacheFileDir, int cacheQueueSize, ProtocolType pt) throws InitializationException {
    ...
    if (!isInit) {
      ...
      if (cacheFileDir == null) {
        throw new InitializationException("There is no available cache log directory.Cache log file path:"
            + cacheFileDir);
      }
      File cacheLogFile = new File(cacheFileDir);
      if (!cacheLogFile.exists()) {
        if (!cacheLogFile.mkdirs()) {
          throw new InitializationException("Can't create cache log directory:" + cacheFileDir);
        }
      }
      ...
      try {
        ...
        throw new InitializationException("Can't initialize cache log writer!");
      }

      logDataBQ = new LinkedList<LogData[]>();
