...
public class EventListener extends Thread{
	...
	public void initialize() throws IOException{
		...
		try {
			...
			logger.error("fail to open storage adapter and eventlistener exist ! the error message is "+e.getMessage(),e);
		}
	}
	...
	public void add(LogMeta logMeta){
		if(logBQ.size()>=queueCapacity){
			if(logger.isDebugEnabled()){
				logger.debug("categories :" + LogDataFormat.transformCategories(logMeta.getLogData().categories)+",queue is full!");
			}
