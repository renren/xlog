...
public class HAClusterManager<K, V> extends ClusterManager<K, V>{
  ...
  public void add(String address) throws KeeperException, InterruptedException, IOException {
    this.address=address;
    int pos=chooseSlot();
    currentZnode=conn.get().create(this.clusterpath + "/" + pos+"/zn_"+pos, address.getBytes(), Ids.OPEN_ACL_UNSAFE,
  ...
  public void createZNode(int slot) throws KeeperException, InterruptedException, IOException{
    String logicZnode=conn.get().create(this.clusterpath + "/" + slot+"/zn_"+slot,this.address.getBytes(), Ids.OPEN_ACL_UNSAFE,
        CreateMode.EPHEMERAL_SEQUENTIAL);
