...
public class HABalancer implements Watcher {
    ...
    private String readAddress(String child) throws KeeperException, InterruptedException,
            IOException {
        byte[] data = conn.get().getData(this.parent + "/" + child, false, null);
        return new String(data);
    }

    private void writeServingItem(int pos, String address) throws KeeperException,
            InterruptedException, IOException {
        conn.get().setData(this.parent + "/" + String.valueOf(pos), address.getBytes(), -1);
    }
